...
<%
    ...
    int count = dao.getArticleCount(1); // categori가 1인 글의 총 개수를 가져옴
    List<ArticleDTO> list = dao.getArticleList(startRow, endRow, 1); // categori가 1인 글의 목록을 가져옴
...
<div class="container mt-4">
    ...
    <table class="table table-hover mt-3">
        ...
        <% if (list == null || list.isEmpty()) { %>
            <tr>
                <%-- ✅ 4. 메시지 수정 --%>
                <td colspan="4" class="text-center">등록된 정치 뉴스가 없습니다.</td>
            </tr>
        <% } else {
            ...
            <tr>
                ...
                <td><a href="politicsView.jsp?num=<%= dto.getNum() %>&pageNum=<%= currentPage %>"><%= dto.getSubject() %></a></td>
                <td><%= dto.getWriter() %></td>
    ...
    <%
        if (count > 0) {
            ...
            if (startPage > 10) {
    %>
                <a href="<%= request.getContextPath() %>/news/politics/politicsList.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
    <%
            }
    ...
    <%
                    ...
                    // 다른 페이지는 링크 설정
    %>
                    <a href="<%= request.getContextPath() %>/news/politics/politicsList.jsp?pageNum=<%= i %>">[<%= i %>]</a>
    <%
                }
            } // for end
            ...
            if (endPage < pageCount) {
    %>
                <a href="<%= request.getContextPath() %>/news/politics/politicsList.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
    <%
            }
        }
    %>
