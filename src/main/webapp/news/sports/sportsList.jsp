...
<%
    ...
    int count = dao.getArticleCount(5); // categori가 5인 글의 총 개수를 가져옴
    List<ArticleDTO> list = dao.getArticleList(startRow, endRow, 5); // categori가 1인 글의 목록을 가져옴
...
<div class="container mt-4">
    <%-- ✅ 3. 화면 제목 수정 --%>
    <h2 class="text-center">스포츠 뉴스</h2>
    ...
    <table class="table table-hover mt-3">
        ...
        <% if (list == null || list.isEmpty()) { %>
            <tr>
                <%-- ✅ 4. 메시지 수정 --%>
                <td colspan="4" class="text-center">등록된 스포츠 뉴스가 없습니다.</td>
            </tr>
        <% } else {
               for (ArticleDTO dto : list) { %>
            <tr>
                <td><%= number-- %></td>
                <td><a href="sportsView.jsp?num=<%= dto.getNum() %>&pageNum=<%= currentPage %>"><%= dto.getSubject() %></a></td>
                <td><%= dto.getWriter() %></td>
    ...
    <%
        if (count > 0) {
            ...
            if (startPage > 10) {
    %>
                <a href="<%= request.getContextPath() %>/news/sports/sportsList.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
    <%
            }
    ...
    <%
                    ...
                    // 다른 페이지는 링크 설정
    %>
                    <a href="<%= request.getContextPath() %>/news/sports/sportsList.jsp?pageNum=<%= i %>">[<%= i %>]</a>
    <%
                }
            } // for end
            ...
            if (endPage < pageCount) {
    %>
                <a href="<%= request.getContextPath() %>/news/sports/sportsList.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
    <%
            }
        }
    %>
